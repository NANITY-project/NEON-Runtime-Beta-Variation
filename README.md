...
- **Vulkan backend** (`rawllm_vulkan.hpp`): experimental, matvec-only
  (no GEMM/batched path — NEON is single-user, single-prompt, so there's
  no batching workload to make one worthwhile), gated behind
  `-DUSE_VULKAN`. Verified correct **and benchmarked on real GPU hardware**
  (NVIDIA T4) — the optimized backend came in about 5–11% slower than
  cuBLAS's SGEMV on the same hardware, a clean isolation of the API/driver
  gap with hardware held constant. See
  [`VULKAN_BACKEND_NOTES.md` on the `VULKAN-EXPERIMENTAL` branch](https://github.com/NANITY-project/NEON-Runtime-Beta-Variation/blob/VULKAN-EXPERIMENTAL/VULKAN_BACKEND_NOTES.md)
  for the full validation process and results. Still **not wired into the
  actual generation hot path**: `--model ... --probe` with a
  Vulkan-enabled build will initialize the backend and log whether a
  usable device was found, but every token is still computed on the CPU
  path (`rawllm_simd_dispatch.hpp`) regardless — deciding when GPU beats
  CPU for a given tensor size and actually routing `proj_all_positions()`
  through it is the next piece of work here, not something this header
  claims to do yet. AMD/ROCm hardware (the project's actual target) hasn't
  been tested yet either — the T4 result used the only GPU available at
  the time.
...
| `rawllm_vulkan.hpp` | Vulkan compute backend (experimental, F32/Q4_0/Q8_0 matvec only — no GEMM, see "Known limitations"). Compiles here under `-DUSE_VULKAN`; the shader sources and build/test workflow are on `VULKAN-EXPERIMENTAL` |
...
| `VULKAN_BACKEND_NOTES.md` | Vulkan backend perf-work log and real-hardware (NVIDIA T4) validation results (~5–11% slower than cuBLAS). Written against the `VULKAN-EXPERIMENTAL` branch's file layout — see that branch's README for the current build/test guide |
